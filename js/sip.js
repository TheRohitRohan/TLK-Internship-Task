...
const monthlyBubble = document.getElementById('monthlyInvestmentValue');
...
function updateBubbles() {
    monthlyBubble.className = 'sip-value-bubble money';
    monthlyBubble.innerHTML = `<span class='bubble-prefix'>₹</span><span class='bubble-number'>${parseInt(monthlyInput.value).toLocaleString('en-IN')}</span>`;
...
function calculateSIP() {
    ...
    const r = parseFloat(returnInput.value) / 100 / 12;
    const n = parseInt(periodInput.value) * 12;
    // SIP formula
...
function updateSummaryAndChart() {
    const { invested, returns, FV } = calculateSIP();
...
// Event Listeners
monthlyInput.addEventListener('input', handleInput);
...
makeEditable(monthlyBubble, monthlyInput, 'money');
