...
const carPriceBubble = document.getElementById('carPriceValue');
const downPaymentBubble = document.getElementById('downPaymentValue');
...
function updateBubbles() {
    carPriceBubble.className = 'value-bubble';
    carPriceBubble.innerHTML = `<span class='bubble-prefix'>₹</span><span class='bubble-number'>${parseInt(carPriceInput.value).toLocaleString('en-IN')}</span>`;
    
    downPaymentBubble.className = 'value-bubble';
    downPaymentBubble.innerHTML = `<span class='bubble-prefix'>₹</span><span class='bubble-number'>${parseInt(downPaymentInput.value).toLocaleString('en-IN')}</span>`;
...
function updateSummary() {
    const { loanAmount, monthlyEMI, totalInterest, totalPayment, yearlyBreakdown } = calculateCarLoan();
    
    loanAmountEl.textContent = formatINR(loanAmount);
...
// Event Listeners
carPriceInput.addEventListener('input', handleInput);
downPaymentInput.addEventListener('input', handleInput);
...
makeEditable(carPriceBubble, carPriceInput, 'money');
makeEditable(downPaymentBubble, downPaymentInput, 'money');
